...
<script lang="ts">
  ...
  import { formSchema } from './ispeakTag.data';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import { addIspeakTag, updateIspeakTag } from '/@/api/ispeak/ispeak';
      ...
      async function handleSubmit() {
        try {
          ...
          if (updateObject.value) {
            // 修改接口
            await updateIspeakTag({ _id: updateObject.value, ...values });
          } else {
            // 新增接口
            await addIspeakTag(values);
          }
          console.log(values);
          // 成功
          closeDrawer();
